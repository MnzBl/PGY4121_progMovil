...
export class CrearViajePage implements OnInit {
  ...
  fnguardarviaje(direccion_i: string, direccion_f: string) {
    ...
    const UrlCoordenadasCompleta_i =
      UrlCoordenadas1 + direccionCompleta_i + UrlCoordenadas2;
    const UrlCoordenadasCompleta_f =
      UrlCoordenadas1 + direccionCompleta_f + UrlCoordenadas2;
      ...
      if (res.length > 0) {
        ...
        this.http.get<any[]>(UrlCoordenadasCompleta_f).subscribe((res) => {
          if (res.length > 0) {
            ...
            this.viaje.fecha = new Date().toISOString();

            console.log('Latitud:', this.viaje.lat_f);
            console.log('Longitud:', this.viaje.lon_f);
            console.log('fecha', this.viaje.fecha);

            this.http
              .post(url, this.viaje, { headers })
